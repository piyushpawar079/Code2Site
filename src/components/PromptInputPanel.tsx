...
interface PromptInputPanelProps {
  ...
  handleGenerate: () => void;
  addNewFeatures: (userPrompt: string) => void;
...
export default function PromptInputPanel({
  ...
  return (
    <div className={`${hasGenerated ? 'w-[30%]' : 'w-full'} border-r border-gray-800 bg-gradient-to-br from-gray-900/50 to-gray-800/30 transition-all duration-500 flex flex-col`}>
      ...
      <div className={`${hasGenerated ? 'border-t border-gray-700 p-4' : 'flex-1 justify-center items-center flex p-6'}`}>
        <div className={`${hasGenerated ? '' : 'max-w-2xl mx-auto text-center'} w-full`}>
          {/* Header - Only show when not generated */}
          {!hasGenerated && (
            <div className="mb-6 text-center">
              <div className="flex items-center gap-2 mb-3 justify-center">
                ...
                <h2 className="text-3xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent transition-all duration-500">
                  AI Website Builder
                </h2>
              </div>
            ...
            <div className="relative">
              <textarea
                ...
                placeholder={hasGenerated ? 
                  "Add more features, modify existing ones, or make changes to your website..." : 
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  <span>{hasGenerated ? 'Send' : 'Generate Website'}</span>
                </>
              )}
            </button>
          </div>

          {/* Feature hints for first time users - Only show when not generated */}
          {!hasGenerated && (
